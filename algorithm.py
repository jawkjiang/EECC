...
class AI_planing:
        ...
        def obj_func(x):
            ...
            for i in range(len(self.H2_power)):
                for j in range(96):
                    ...
                    # 氢能源容量序列
                    globals()[f'x{i + len(self.H2_power)}_{j}'] = x[96 * len(self.H2_power) + i * 96 + j]

            for i in range(len(self.ele_power)):
                for j in range(96):
                    # 电化学出力序列
                    globals()[f'y{i}_{j}'] = x[96 * 2 * len(self.H2_power) + i * 96 + j]
                    # 电化学容量序列
                    globals()[f'y{i + len(self.ele_power)}_{j}'] = x[
                        96 * 2 * len(self.H2_power) + 96 * len(self.ele_power) + i * 96 + j]

            for i in range(len(self.water_power)):
                for j in range(96):
                    # 抽水蓄能出力序列
                    globals()[f'z{i}_{j}'] = x[96 * 2 * len(self.H2_power) + 96 * 2 * len(self.ele_power) + i * 96 + j]
                    # 抽水蓄能容量序列
                    globals()[f'z{i + len(self.water_power)}_{j}'] = x[
                        96 * 2 * len(self.H2_power) + 96 * 2 * len(self.ele_power) +
                        96 * len(self.water_power) + i * 96 + j]
 ...
 \
                    ...
                    + 102e4 * sum([(3e-4 - 0.009 * (eval(f"y{i + len(self.ele_power)}_{j}") / 60 / self.ele_power[i]
                                                    - (1 - 1 / 3000) ** (1 / 2.66) * self.ele_power[i] / 60 / 15)) *
                                   self.ele_power[
                                       i] / 365 / 96 / 1e3
                                   for i in range(len(self.ele_power)) for j in range(96)]) + \
 \
                    100 * sum(eval(f"abs(z{i}_{j})") for i in range(len(self.water_power)) for j in range(96))

        self.obj_func = obj_func
        self.n_dim = 2 * 96 * (len(self.H2_power) + len(self.ele_power) + len(self.water_power))
        ...
        for i in range(len(self.ele_power)):
            for j in range(96):
                self.ub.append(1.6 * self.ele_power[i])
                self.lb.append(0.4 * self.ele_power[i])
        for i in range(len(self.water_power)):
            for j in range(96):
                    ...
                    self.constraint_eq += (
                        lambda x: eval(f"z{i + len(self.water_power)}_{j}") - eval(
                            f"z{i + len(self.water_power)}_{j - 1}") +
                                  eval(f"max(x{i}_{j}, 0.75 * x{i}_{j})") * 0.25 * 15 * 60 / 2000 / 9.8 / 1000 * 1e6,)
        # 潮流缺额约束
        for j in range(96):
            self.constraint_eq += (
        ...
        for i in range(len(water_power)):
            for j in range(96):
                constraint_ueq += (lambda x: eval(f"z{i + len(water_power)}_{j}") - 1146.5 * 1e4,)
                constraint_ueq += (lambda x: -eval(f"z{i + len(water_power)}_{j}"),)
        '''
        self.best_x = None
        self.best_y = None
    ...
    def GA_planning(self, size_pop, max_iter, prob_mut, precision):
        result = GA(func=self.obj_func, n_dim=self.n_dim, size_pop=size_pop, max_iter=max_iter, prob_mut=prob_mut,
                    lb=self.lb, ub=self.ub, constraint_eq=self.constraint_eq, precision=precision)
        return result
    ...
    def PSO_planning(self, size_pop, max_iter, w, c1, c2):
        result = PSO(func=self.obj_func, n_dim=self.n_dim, pop=size_pop, max_iter=max_iter,
                     lb=self.lb, ub=self.ub, w=w, c1=c1, c2=c2, constraint_eq=self.constraint_eq)
        return result
    ...
    def DE_planning(self, size_pop, max_iter, prob_mut, F):
        result = DE(func=self.obj_func, n_dim=self.n_dim, size_pop=size_pop, max_iter=max_iter, prob_mut=prob_mut, F=F,
                    lb=self.lb, ub=self.ub, constraint_eq=self.constraint_eq)
        return result
    ...
    def calculate(self, method, times, **kargs):
        ...
        if method == self.SA_planning or method == self.AFSA_planning:
            best_x = 1000 * best_x
            best_y = 1000 * best_y
        ...
        best_x = best_x[0: len(self.H2_power)] + \
                 best_x[2 * len(self.H2_power): 2 * len(self.H2_power) + len(self.ele_power)] + \
                 best_x[2 * len(self.H2_power) + 2 * len(self.ele_power): 2 * len(self.H2_power) + 2 * len(
...
def main():
    loss_list = loss_read("data/潮流缺额与剩余随时间变化数据.csv")
    station_list = station_read("data/湖南.csv")
